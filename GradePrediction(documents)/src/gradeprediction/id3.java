...
public class id3 extends javax.swing.JFrame {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        try {
            ...
            ResultSet rs = st.executeQuery("select distinct(Timetaken) from cluster2");
            while (rs.next()) {
                String s = rs.getString(1);
                ResultSet r = st1.executeQuery("select count(*) from cluster2 where Grade='" + s + "'");
                while (r.next()) {
                    cnt = r.getInt(1);
                    jTextArea2.append(cnt + "\n");
                }
                String c = String.valueOf(cnt);
                st2.executeUpdate("insert into id31 values('" + s + "','" + cnt + "')");
            }
            jTextArea2.append("\nP(A)\n*******\n");
            ResultSet rs1 = st.executeQuery("select distinct(Timetaken) from cluster2");
            while (rs1.next()) {
                int s = rs1.getInt(1);
                ResultSet r = st1.executeQuery("select count(*) from cluster2 where Timetaken='" + s + "'");
                while (r.next()) {
                    cnt = r.getInt(1);
                    jTextArea2.append(cnt + "\n");
                }
                String c = String.valueOf(cnt);
                st2.executeUpdate("insert into id32 values('" + s + "','" + c + "')");
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
...
//            jTextArea3.append("Cluster1\tCluster2\tCluster3\n");
            ...
            ResultSet rs = st.executeQuery("select distinct(Timetaken) from cluster1");
            while (rs.next()) {
                int s = rs.getInt(1);
                ResultSet r = st1.executeQuery("select distinct(Grade) from cluster1 where Timetaken='" + s + "'");
                while (r.next()) {
                    String s1 = r.getString(1);
                    ResultSet r1 = st2.executeQuery("select count(*) from cluster1 where Timetaken='" + s + "' and Grade='" + s1 + "'");
                    while (r1.next()) {
