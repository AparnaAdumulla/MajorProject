...
public class calculation extends javax.swing.JFrame {
    ...
    public calculation() {
        ...
        try {
            ...
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/ecb", "root", "");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {
            ...
            int i = 0;
            int j = 1;
            jTextArea1.append("Attribute\tClass\tCluster1\tCluster2\tCluster3\n");
            int[] a = new int[10];
            st.executeUpdate("TRUNCATE TABLE classify");
            ResultSet rs = st.executeQuery("select * from clustering");
            ...
            for (int k = 1; k < rmd.getColumnCount() - 1; k++) {
                ResultSet rrr = st4.executeQuery("select distinct(n) from clustering");
                while (rrr.next()) {
                    String aa = rrr.getString(1);
                    jTextArea1.append(col[k] + "\t" + aa + "\t");
                    ResultSet r = st1.executeQuery("select distinct(e) from clustering where n='" + aa + "'");
                    while (r.next()) {
                        String s1 = r.getString(1);
                        ResultSet r1 = st2.executeQuery("select count('" + col[k] + "') from clustering where n='" + aa + "' and e='" + s1 + "'");
                        while (r1.next()) {
                    ...
                    st2.executeUpdate("insert into classify values('" + col[k] + "','" + aa + "'," + a[0] + "," + a[1] + "," + a[2] + ")");
                    j++;
                    i = 0;
                    jTextArea1.append("\n");
                }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        try {
            int i = 1;
            jTextArea2.append("Attribute\tClass\tcluster1\tcluster2\tInformation Gain\n");
...
//                re = -(a1 * Math.log10(a1)) - (b1 * Math.log10(b1)) - (c1 * (Math.log10(c1)));
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        try {
            ...
            int i = 0, j = 0;
            double s = 0;
            ResultSet rs = st.executeQuery("select distinct(attribute) from classify");
            while (rs.next()) {
                String aa = rs.getString(1);
                ResultSet ress = st2.executeQuery("select distinct(cluster1) from classify where attribute='" + aa + "'");
                while (ress.next()) {
                    String s1 = ress.getString(1);
                    ResultSet r = st1.executeQuery("select * from classify where attribute='" + aa + "'and cluster1='" + s1 + "'");
                    while (r.next()) {
                        a = r.getInt(3);
                        b = r.getInt(4);
                        c = r.getInt(5);
            ...
            rs = st.executeQuery("select distinct(attribute) from classify");
            while (rs.next()) {
                String aa = rs.getString(1);
                ResultSet ress = st2.executeQuery("select distinct(cluster1) from classify where attribute='" + aa + "'");
                while (ress.next()) {
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        try {
            int i = 0;
            ResultSet rr = st1.executeQuery("select distinct(attribute) from classify");
            while (rr.next()) {
                String aa = rr.getString(1);
                System.out.println(aa);
                ResultSet rs = st.executeQuery("select sum(cluster1),sum(class) from classify where attribute='" + aa + "'");
                while (rs.next()) {
                    a = rs.getInt(1);
                    double a1 = a / sum;
                    double b1 = b / sum;
...
//                    c = rs.getInt(3);
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        try {
            rs = st1.executeQuery("select n,v,i,e from clustering");
            while (rs.next()) {
